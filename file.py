...
if __name__=='__main__':
    ...
    # 选择文件
    b1 = tk.Button(window,text="选择加密文件",command=chooseFile,font="Helvetica 10 ",width=11)
    b1.grid(row=0,column=1)
    encryptFile = tk.Text(window,height=1,width=34,font="Helvetica 10 ")
    encryptFile.grid(row=0,column=2,padx=5, pady=10,sticky='w')

    # 密钥
    label = tk.Label(text = '加密密钥：',font="Helvetica 10 ")
    label.grid(row=1,column=0,padx=5, pady=10)
    key = tk.Entry(window,width=50,font="Helvetica 10 ")
    key.grid(row=1,column=1,columnspan=2,padx=5, pady=10,sticky='w')

    # 加密选中的文件按钮
    b2 = tk.Button(window,text="加密文件",command=encrypt,font="Helvetica 10 ",width=11)
    b2.grid(row=2,column=1,padx=5, pady=10)

    # 解密已加密的文件按钮
    b2 = tk.Button(window,text="解密文件",command=decrypt,font="Helvetica 10 ",width=11)
    b2.grid(row=2,column=2,padx=5, pady=10)

    # 注
    labelNotice = tk.Label(text = '注：加密和解密生成的文件生成在源文件同一目录下！',font="Helvetica 10 ")
    labelNotice.grid(row=3,column=0,columnspan=3,padx=5, pady=10)
